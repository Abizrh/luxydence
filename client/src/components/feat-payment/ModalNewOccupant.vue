...
const titleName: any = computed(() => (modeEditor.value === 'new' ? 'New house' : 'Edit house'))
const optionsStatus = [
	{ label: 'Kosong', value: 'kosong' },
	{ label: 'Tetap', value: 'tetap' },
	{ label: 'Kontrak', value: 'contract' },
]
const models: any = ref({ house_number: 0, status: '', occupant_id: null })
const families = ref<OccupantListChild[]>()
const errors: any = ref({ house_number: 0, status: '', occupant_id: null })
		...
		<q-card class="half-rounded tw-min-w-[450px] tw-p-4">
			...
			<q-card-section class="q-pt-none tw-mt-4 tw-flex tw-h-[calc(100vh-130px)] tw-flex-col">
				<div class="tw-mt-4 tw-flex-auto tw-flex-col">
					...
					/>
					<span class="tw-text-body-2 tw-font-semibold">Pemilik Rumah</span>
					...
					<q-select v-model="models.status" :options="optionsStatus" outlined hide-dropdown-icon input-debounce="0" dense class="q-mb-sm">
						<template #append>
							<q-icon name="arrow_down" size="20px" class="black-filter primary-filter"></q-icon>
						</template>
					</q-select>
