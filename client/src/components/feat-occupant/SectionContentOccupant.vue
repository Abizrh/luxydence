...
const columns: any = ref([
	...
	{ id: 'responsible', label: 'Pemilik', field: 'responsible', name: 'responsible', sortable: false, align: 'left' },
...
const setRowContent = async (value: any) => {
	...
	for (const item of lists) {
		const { id, responsible, families } = item
		const objects = {
			id,
			responsible,
...
const getRequest = async (parameters: any = null) => {
	const resultContactList = await createRequest('occupants', 'get', parameters)
...
const onRemoveByCheck = async () => {
	...
	for (const item of filters) {
		await createRequest('occupants', 'delete', null, { sub_path: item })
	}
	await sleep(100)
	await onRequest()
}
	...
	<div class="tw-flex tw-min-h-[calc(100vh-40px)] tw-w-full tw-flex-col tw-gap-2 tw-p-2">
		<div class="tw-flex tw-items-center tw-justify-between">
			<div class="tw-flex tw-items-center tw-gap-8">
				...
				<span class="tw-text-[12px] tw-font-light">({{ totalCount }} occupants)</span>
			</div>
			<div class="tw-flex tw-gap-2">
				...
				<q-btn color="primary" class="tw-w-[164px]" @click="onClickNew">
					...
					<span class="tw-whitespace-nowrap tw-capitalize tw-text-white">Penghuni</span>
				</q-btn>
