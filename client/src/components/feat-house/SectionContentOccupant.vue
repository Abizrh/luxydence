...
const columns: any = ref([
	...
	{ id: 'owner', label: 'Pemilik Rumah', field: 'owner', name: 'owner', sortable: false, align: 'left' },
	{ id: 'status', label: 'Status Rumah', field: 'status', name: 'status', sortable: false, align: 'left' },
	{ id: 'responsible', label: 'Kepala Penghuni', field: 'responsible', name: 'responsible', sortable: false, align: 'left' },
	{ id: 'families', label: 'Penghuni', field: 'families', name: 'families', sortable: false, align: 'left' },
])
const rows: any = ref([])
...
const setRowContent = async (value: any) => {
	...
	for (const item of lists) {
		const { id, house_number, status, owner } = item
		const objects = {
			id,
			house_number,
			responsible: item?.Occupant?.responsible ?? '-',
			families: JSON.parse(item?.Occupant?.families ?? '[]'),
			status,
			owner,
		}
...
const getRequest = async (parameters: any = null) => {
	const resultContactList = await createRequest('houses', 'get', parameters)
...
const onRemoveByCheck = async () => {
	...
	for (const item of filters) {
		await createRequest('houses', 'delete', null, { sub_path: item })
	}
	await sleep(100)
	await onRequest()
}
	...
	<div class="tw-flex tw-min-h-[calc(100vh-40px)] tw-w-full tw-flex-col tw-gap-2 tw-p-2">
		<div class="tw-flex tw-items-center tw-justify-between">
			<div class="tw-flex tw-items-center tw-gap-8">
				...
				<span class="tw-text-[12px] tw-font-light">({{ totalCount }} rumah)</span>
			</div>
			<div class="tw-flex tw-gap-2">
				<div class="tw-flex tw-h-[36px] tw-w-[39px] tw-flex-none tw-cursor-pointer tw-items-center tw-justify-center tw-rounded-[7px] tw-bg-white">
					...
					<q-tooltip :delay="100" :offset="[0, 10]">Filter</q-tooltip>
				</div>
				<q-btn color="primary" class="tw-w-[164px]" @click="onClickNew">
					...
					<span class="tw-whitespace-nowrap tw-capitalize tw-text-white">Rumah</span>
				</q-btn>
		...
		>
			...
			<template #body-cell="props">
				<q-td :props="props" class="tw-cursor-pointer" @click="onClickDetail(props.col.id, props.row.id || '')">
					<span v-if="!['check', 'families'].includes(props.col.id)" class="tw-font-light">{{ props.value || '-' }}</span>
					<div v-else-if="props.col.id === 'families'">
						<ul v-for="(oc, idx) in props?.value" :key="idx" class="tw-list-disc">
							<li>{{ oc.name }}</li>
						</ul>
					</div>
